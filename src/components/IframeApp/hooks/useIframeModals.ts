...
export type ModalType = 'auth' | 'signing' | 'addAuth' | 'removeAuth' | 'grant' | null;
...
export function useIframeModals() {
  ...
  const openModal = useCallback((type: ModalType, payload?: any, resolver?: (value: any) => void) => {
    setModalState({
      type,
      payload,
      resolver,
  ...
  const transitionToModal = useCallback((type: ModalType, payload?: any, resolver?: (value: any) => void) => {
  ...
  // Auth modal handlers
  const openAuthModal = useCallback((resolver: (value: any) => void) => {
    openModal('auth', undefined, resolver);
  }, [openModal]);

  const closeAuthModal = useCallback((address?: string) => {
    if (address) {
      closeModal({ address });
    } else {
      closeModal();
  ...
  const openSigningModal = useCallback((transaction: SignTransactionPayload['transaction'], resolver: (value: any) => void) => {
  ...
  const closeSigningModal = useCallback((result?: any) => {
    closeModal(result);
  }, [closeModal]);

  // Add authenticator modal handlers
  const openAddAuthModal = useCallback((payload: AddAuthenticatorPayload, resolver: (value: any) => void) => {
    openModal('addAuth', payload, resolver);
  }, [openModal]);

  const closeAddAuthModal = useCallback((result?: any) => {
    closeModal(result);
  }, [closeModal]);

  // Remove authenticator modal handlers
  const openRemoveAuthModal = useCallback((authenticatorId: number, resolver: (value: any) => void) => {
    openModal('removeAuth', { authenticatorId }, resolver);
  }, [openModal]);

  const closeRemoveAuthModal = useCallback((result?: any) => {
    closeModal(result);
  }, [closeModal]);

  // Grant modal handlers
  const openGrantModal = useCallback((treasuryAddress: string, grantee: string, resolver: (value: any) => void) => {
    openModal('grant', { treasuryAddress, grantee }, resolver);
  }, [openModal]);

  const closeGrantModal = useCallback((success: boolean) => {
    closeModal({ success });
  }, [closeModal]);
  ...
  useEffect(() => {
    ...
    console.log('[useIframeModals] Modal state change:', { type: modalState.type, hasModalOpen });
