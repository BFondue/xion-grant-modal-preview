...
export async function createJwtAbstractAccount(
  ...
  if (!response.ok) {
    ...
    console.error(
      "[AccountCreation] AA API error:",
      response.status,
      errorBody,
    );
    throw new Error(
      `Failed to create abstract account: ${response.status} ${errorBody}`,
    );
  }
  ...
  console.log(
    ...
    isNewAccount ? "CREATED" : "RETRIEVED",
    ":",
    result.account_address,
  );
  console.log(
    "[AccountCreation] Transaction hash:",
    result.transaction_hash,
    isNewAccount ? "(real tx)" : "(addressHash - existing account)",
  );
