...
import { AbstractAccountJWTSigner } from "../../signers/signers/jwt-signer";
...
describe("AbstractAccountJWTSigner", () => {
  ...
  it("should initialize correctly", () => {
    const signer = new AbstractAccountJWTSigner(mockAccount, mockIndex, mockToken);
  ...
  it("should return empty accounts if abstract account is undefined", async () => {
    const signer = new AbstractAccountJWTSigner(undefined as any, mockIndex, mockToken);
    const accounts = await signer.getAccounts();
  ...
  it("should sign direct", async () => {
    ...
    expect(response.signature.signature).toBe(Buffer.from("mock-jwt").toString("base64"));
  ...
  it("should throw error if session token is undefined", async () => {
    const signer = new AbstractAccountJWTSigner(mockAccount, mockIndex, undefined);
    const mockSignDoc = SignDoc.fromPartial({});
    await expect(signer.signDirect("user", mockSignDoc)).rejects.toThrow("stytch session token is undefined");
  });
  ...
  it("should throw error if fetch fails", async () => {
    ...
    await expect(signer.signDirect("user", mockSignDoc)).rejects.toThrow("Failed to authenticate with stytch");
  });
  ...
  it("should throw error if no session_jwt in response", async () => {
    ...
    await expect(signer.signDirect("user", mockSignDoc)).rejects.toThrow("No session_jwt in response");
  });
  ...
  it("should throw error in signDirectArb if session token is undefined", async () => {
    const signer = new AbstractAccountJWTSigner(mockAccount, mockIndex, undefined);
    await expect(signer.signDirectArb("msg")).rejects.toThrow("stytch session token is undefined");
  });
  ...
  it("should throw error in signDirectArb if fetch fails", async () => {
    ...
    await expect(signer.signDirectArb("msg")).rejects.toThrow("Failed to authenticate with stytch");
  });
  ...
  it("should throw error in signDirectArb if no session_jwt", async () => {
    ...
    await expect(signer.signDirectArb("msg")).rejects.toThrow("No session_jwt in response");
  });
});
